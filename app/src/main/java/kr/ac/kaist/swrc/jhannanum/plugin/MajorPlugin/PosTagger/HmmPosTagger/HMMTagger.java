...
public class HMMTagger implements PosTagger {
	...
	public void initialize(String baseDir, String configFile, HashMap<String,InputStream> hmap_resources) throws Exception  {
		...
		InputStream is = hmap_resources.get("HmmPosTagger");
		InputStreamReader isr = new InputStreamReader(is);
		JSONReader json = new JSONReader(configFile, is);

		InputStream is_pttpos =		hmap_resources.get("PTTpos");
		InputStream is_pttwp  =hmap_resources.get("PTTwp");
		InputStream is_pwtpos  = hmap_resources.get("PWTpos");
		...
		pwt_pos_tf = new ProbabilityDBM(PWT_POS_TDBM_FILE,is_pwtpos);
		ptt_wp_tf = new ProbabilityDBM(PTT_WP_TDBM_FILE,is_pttwp);
		ptt_pos_tf = new ProbabilityDBM(PTT_POS_TDBM_FILE,is_pttpos);
	}
