...
public class ChartMorphAnalyzer implements MorphAnalyzer {
	 ...
	 */
	@Override
	public void initialize(String baseDir, String configFile, HashMap<String,InputStream> hmap_resources) throws Exception {
		InputStream is = hmap_resources.get("ChartMorphAnalyzer");
		JSONReader json = new JSONReader(configFile, is);
		...
		tagSet = new TagSet();
		tagSet.init(TagSet.TAG_SET_KAIST, hmap_resources);
//		tagSet.init(fileTagSet, TagSet.TAG_SET_KAIST, hmap_resources);

		connection = new Connection();
		connection.init(fileConnections, tagSet.getTagCount(), tagSet,hmap_resources);

		connectionNot = new ConnectionNot();
		connectionNot.init(fileConnectionsNot, tagSet,hmap_resources);

		analyzedDic = new AnalyzedDic();
		InputStream is_dic_analyzed = hmap_resources.get("dic_analyzed");
		analyzedDic.readDic(fileDicAnalyzed,is_dic_analyzed);

		systemDic = new Trie(Trie.DEFAULT_TRIE_BUF_SIZE_SYS);
		InputStream is_dic_system = hmap_resources.get("dic_system");
		systemDic.read_dic(fileDicSystem, tagSet,is_dic_system);

		userDic = new Trie(Trie.DEFAULT_TRIE_BUF_SIZE_USER);
		InputStream is_dic_user= hmap_resources.get("dic_user");
		userDic.read_dic(fileDicUser, tagSet,is_dic_user);
